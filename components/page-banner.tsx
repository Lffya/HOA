      ...
      <div className="relative z-10 text-center px-6 max-w-5xl mx-auto">
        ...
        {breadcrumbs.length > 0 && (
          <nav className="flex items-center justify-center space-x-2 mb-6 text-sm">
            <Link
              href="/"
              className="flex items-center text-white/70 hover:text-hoa-gold transition-colors duration-300"
            >
            ...
            {breadcrumbs.map((item, index) => (
              <div key={index} className="flex items-center space-x-2">
                ...
                {item.href ? (
                  <Link href={item.href} className="text-white/70 hover:text-hoa-gold transition-colors duration-300">
                    {item.name}
                  </Link>
                ) : (
                  <span className="text-hoa-gold font-medium">{item.name}</span>
                )}
              </div>
            ))}
          </nav>
        ...
        <h1 className="text-5xl md:text-7xl font-bold text-white mb-6 font-serif tracking-wide animate-fade-in-up">
          {title}
        </h1>
